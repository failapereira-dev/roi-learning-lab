...
function populateLoginUserSelect() {
    ...
    profOpt.innerText = "Professor (F. Santos)";
...
function setupWhiteboardCanvas() {
    ...
    if (appState.currentUser === 'professor') {
        ...
        if (status) {
            status.innerHTML = `<i class="fa-solid fa-pen"></i> Professor Desenhando`;
...
function setupEventListeners() {
    ...
    document.getElementById("formActivity1").addEventListener("submit", async (e) => {
        ...
        await submitAssignment("class1", groupId, data);
    });
    ...
    document.getElementById("formActivity2").addEventListener("submit", async (e) => {
        ...
        await submitAssignment("class2", groupId, data);
    });
    ...
    document.getElementById("formActivity3").addEventListener("submit", async (e) => {
        ...
        await submitAssignment("class3", groupId, data);
    });
...
function updateWorkspaceView() {
    ...
    if (isProfessor) {
        ...
        if (appState.activeTab === "groupwork") {
            if (appState.activeClassId === "class1" && activity1Container) {
                activity1Container.style.display = "block";
            } else if (appState.activeClassId === "class2" && activity2Container) {
                activity2Container.style.display = "block";
                if (scenario) {
                    setupSimulatorInputs(scenario);
                }
            } else if (appState.activeClassId === "class3" && activity3Container) {
                    ...
                    modControlsHTML = `
                        <div class="mod-controls-row">
                            <button class="btn-mod-action btn-pin-toggle ${ans.pinned ? 'active-pin' : ''}" data-id="${ans.id}" title="Pin/Desfixar">
                                <i class="fa-solid fa-thumbtack"></i> ${ans.pinned ? 'Desfixar' : 'Pin'}
                            </button>
        ...
        Object.entries(reflections).forEach(([grpId, data]) => {
            ...
            card.innerHTML = `
                <div class="comment-meta">
                    ...
                    <span>${new Date(data.submittedAt).toLocaleTimeString("en-US", { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
    ...
    try {
        ...
        if (comments.length > 0) {
            ...
            comments.forEach(comment => {
                ...
                const dateStr = new Date(comment.timestamp).toLocaleTimeString("en-US", { hour: '2-digit', minute: '2-digit' });
            ...
            subContentHTML = `
                ...
                <div class="inspect-roi-grid">
                    <div class="inspect-roi-card expected">
                        ...
                        <small>${submission.metrics && submission.metrics.expected ? submission.metrics.expected.vpl : '$0'}</small>
                    </div>
                    <div class="inspect-roi-card realistic">
                        ...
                        <small>${submission.metrics && submission.metrics.realistic ? submission.metrics.realistic.vpl : '$0'}</small>
                    </div>
                    <div class="inspect-roi-card pessimistic">
                        ...
                        <small>${submission.metrics && submission.metrics.pessimistic ? submission.metrics.pessimistic.vpl : '$0'}</small>
                    </div>
                </div>
